  ...
  >
    ...
    <div class="relative h-52 overflow-hidden">
      <img
        ...
        :alt="i18n.pick(props.destination.name)"
      ...
      <span class="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-700 backdrop-blur dark:bg-slate-900/90 dark:text-slate-200">
        {{ i18n.categoryLabel(props.destination.category) }}
      </span>
      <button
        type="button"
        ...
        :aria-label="
          favoriteStore.isFavorite(props.destination.id)
    ...
    <div class="flex flex-1 flex-col p-5">
      <div class="flex items-start justify-between gap-2">
        <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ i18n.pick(props.destination.name) }}</h3>
      ...
      <div class="mt-4 flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2 text-sm dark:bg-slate-800/60">
        <span class="flex items-center gap-1 font-semibold text-slate-700 dark:text-slate-200">
          ...
          {{ i18n.pick(props.destination.bestTimeToVisit).split('(')[0].trim() }}
        </span>
        <span class="font-bold text-teal-600 dark:text-teal-400">${{ props.destination.budget }}{{ i18n.t('common.perDay') }}</span>
      </div>
