...
function submitRegister(): void {
  ...
  if (!name) {
    notify(i18n.t('auth.errName'), 'error')
    return
  }
  if (!form.email.includes('@')) {
    notify(i18n.t('auth.errEmail'), 'error')
    return
  }
  if (form.password.length < 4) {
    notify(i18n.t('auth.errPasswordLength'), 'error')
    return
  }
  if (form.password !== form.confirmPassword) {
    notify(i18n.t('auth.errPasswordMismatch'), 'error')
    return
  }
  ...
  notify(i18n.t('auth.welcomeAboard', { name }))
...
<template>
  <div class="mx-auto flex max-w-lg flex-col px-4 py-16 sm:px-6">
    <div class="mb-6 text-center">
      ...
      <h1 class="mt-4 text-3xl font-bold text-slate-900 dark:text-white">{{ i18n.t('auth.createAccount') }}</h1>
    ...
    >
      <!-- Full name -->
      <label class="block">
        <span class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">{{ i18n.t('auth.fullName') }}</span>
        <div class="relative">
      ...
      <label class="mt-5 block">
        <span class="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-200">{{ i18n.t('auth.confirmPassword') }}</span>
        <div class="relative">
          <Lock :size="18" class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
          <input
          ...
          <button
            type="button"
            class="absolute right-3 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:text-slate-600 dark:hover:text-slate-200"
            :aria-label="showConfirm ? i18n.t('auth.hidePassword') : i18n.t('auth.showPassword')"
      ...
      >
        <LogIn :size="18" />
        {{ i18n.t('auth.createAccountButton') }}
      </button>
    </form>

    <p class="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
      {{ i18n.t('auth.alreadyHave') }}
